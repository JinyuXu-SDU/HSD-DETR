...
#     sub_labels = ['(a)', '(b)', '(c)', '(d)']
#     for i in range(4):
#         # 针对第一个子图(i==0)，下方因为有旋转的类别名，所以字母放低一点(-0.4)
#         # 针对其他子图，字母位置正常(-0.2)
#         y_offset = -0.4 if i == 0 else -0.2
#         ax[i].text(0.5, y_offset, sub_labels[i], transform=ax[i].transAxes,
...
#     """
#     import matplotlib.pyplot as plt  # scope for faster 'import ultralytics'
#     import matplotlib.patches as patches  # 新增：用于绘制矢量矩形框
#     import polars
#     from matplotlib.colors import LinearSegmentedColormap
#     import numpy as np
#     import warnings
...
#     warnings.filterwarnings("ignore", category=FutureWarning)
#
#     # Plot dataset labels - 修改输出文件名为 .svg
#     LOGGER.info(f"Plotting labels to {save_dir / 'labels.svg'}... ")
#     nc = int(cls.max() + 1)  # number of classes
#     boxes_limit = boxes[:1000000]  # limit to 1M boxes
#     x = polars.DataFrame(boxes_limit, schema=["x", "y", "width", "height"])
#
#     # Matplotlib labels
#     subplot_3_4_color = LinearSegmentedColormap.from_list("white_blue", ["white", "blue"])
...
#     y = ax[0].hist(cls, bins=np.linspace(0, nc, nc + 1) - 0.5, rwidth=0.8)
#     for i in range(nc):
#         y[2].patches[i].set_color([val / 255 for val in colors(i)])
#     ax[0].set_ylabel("instances")
#     if 0 < len(names) < 30:
#         ax[0].set_xticks(range(len(names)))
#         ax[0].set_xticklabels(list(names.values()), rotation=90, fontsize=10)
...
#         ax[0].set_xlabel("classes")
#
#     # (b) Bounding box visualization (替换 PIL 为 Matplotlib 原生矢量绘制)
#     ax[1].set_xlim(0, 1000)
#     ax[1].set_ylim(1000, 0)  # 翻转 Y 轴以匹配原先 PIL 图像的坐标系（原点在左上角）
#     ax[1].set_aspect('equal')
#     ax[1].axis("off")
#
#     # 计算原点在 (0.5, 0.5) 时的 [x_min, y_min] 和 [width, height]，并缩放 1000 倍
#     w_h = boxes_limit[:, 2:4] * 1000
#     x_min_y_min = (0.5 - boxes_limit[:, 2:4] / 2) * 1000
#
#     # 使用 matplotlib 原生补丁绘制矩形（真正的矢量线条）
#     for c, xy, wh in zip(cls[:500], x_min_y_min[:500], w_h[:500]):
#         color = [val / 255 for val in colors(int(c))]
#         rect = patches.Rectangle((xy[0], xy[1]), wh[0], wh[1],
#                                  linewidth=1, edgecolor=color, facecolor='none')
#         ax[1].add_patch(rect)
#
#     # (c) 2D Histograms for x, y
#     ax[2].hist2d(x["x"].to_numpy(), x["y"].to_numpy(), bins=50, cmap=subplot_3_4_color)
#     ax[2].set_xlabel("x")
#     ax[2].set_ylabel("y")
#
#     # (d) 2D Histograms for width, height
#     ax[3].hist2d(x["width"].to_numpy(), x["height"].to_numpy(), bins=50, cmap=subplot_3_4_color)
#     ax[3].set_xlabel("width")
#     ax[3].set_ylabel("height")
#
#     for a in {0, 1, 2, 3}:
...
#
#     # 保存为 SVG 矢量图格式
#     fname = save_dir / "labels.svg"  # 改为 .svg 或 .pdf
#     plt.savefig(fname, format="svg", bbox_inches="tight")  # 移除了 dpi=200，并指定格式
#     plt.close()
#
#     if on_plot:
#         on_plot(fname)
...
#     """
#     import matplotlib.pyplot as plt  # scope for faster 'import ultralytics'
#     import matplotlib.patches as patches  # 用于绘制矢量矩形框
#     import polars
#     import numpy as np
#     import warnings
#
...
#     warnings.filterwarnings("ignore", category=FutureWarning)
#
#     # Plot dataset labels - 输出文件名为 .svg
#     LOGGER.info(f"Plotting labels to {save_dir / 'labels.svg'}... ")
#     nc = int(cls.max() + 1)  # number of classes
#     boxes_limit = boxes[:1000000]  # limit to 1M boxes
#     x = polars.DataFrame(boxes_limit, schema=["x", "y", "width", "height"])
...
#         ax[0].set_xticklabels(list(names.values()), rotation=90, fontsize=10)
#         # 恢复顶部数值标签，使其和图一表现一致
#         ax[0].bar_label(y[2], fmt='%g', fontsize=8, padding=2)
#     else:
#         ax[0].set_xlabel("classes")
#
#     # (b) Bounding box visualization (使用 Matplotlib 原生矢量绘制)
#     ax[1].set_xlim(0, 1000)
#     ax[1].set_ylim(1000, 0)  # 翻转 Y 轴以匹配原先 PIL 图像的坐标系（原点在左上角）
#     ax[1].set_aspect('equal')
#     ax[1].axis("off")
...
#
#     # (c) 2D Histograms for x, y
#     # 改用 "Blues" 颜色映射，并增加 cmin=1，使得空白区域透明，低频区域能被清晰地渲染
#     ax[2].hist2d(x["x"].to_numpy(), x["y"].to_numpy(), bins=50, cmap="Blues", cmin=1)
#     ax[2].set_xlabel("x")
#     ax[2].set_ylabel("y")
#
#     # (d) 2D Histograms for width, height
#     # 同上更改，恢复原版点阵状直方图效果
#     ax[3].hist2d(x["width"].to_numpy(), x["height"].to_numpy(), bins=50, cmap="Blues", cmin=1)
#     ax[3].set_xlabel("width")
#     ax[3].set_ylabel("height")
#
#     for a in {0, 1, 2, 3}:
...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    ...
    import matplotlib.patches as patches  # 保持矢量图绘制
    import numpy as np
    import warnings
    ...
    # Plot dataset labels - 输出为 svg 保持矢量图特性
    LOGGER.info(f"Plotting labels to {save_dir / 'labels.svg'}... ")
    nc = int(cls.max() + 1)  # number of classes
    boxes_limit = boxes[:1000000]  # limit to 1M boxes
    x = pd.DataFrame(boxes_limit, columns=['x', 'y', 'width', 'height'])

    # Seaborn correlogram (恢复这部分并保存为矢量图)
    sn.pairplot(x, corner=True, diag_kind='auto', kind='hist', diag_kws=dict(bins=50), plot_kws=dict(pmax=0.9))
    plt.savefig(save_dir / 'labels_correlogram.svg', format="svg", bbox_inches='tight')
    plt.close()
    ...
    for p in ax[0].patches:
        height = p.get_height()
        if height > 0:  # 当有数据时才显示
            ax[0].annotate(f"{int(height)}",
                           (p.get_x() + p.get_width() / 2., height),
                           ha='center', va='center', xytext=(0, 5),
                           textcoords='offset points', fontsize=8)

    # 恢复使用 Seaborn 绘制 (c) 和 (d)
    sn.histplot(x, x='x', y='y', ax=ax[2], bins=50, pmax=0.9)
    sn.histplot(x, x='width', y='height', ax=ax[3], bins=50, pmax=0.9)

    # (b) Bounding box visualization (保持 Matplotlib 原生矢量绘制，不退回 PIL)
    ax[1].set_xlim(0, 1000)
    ax[1].set_ylim(1000, 0)
    ax[1].set_aspect('equal')
    ax[1].axis("off")

    # 矢量化计算坐标点 (不再需要 ops 依赖)
    w_h = boxes_limit[:, 2:4] * 1000
    x_min_y_min = (0.5 - boxes_limit[:, 2:4] / 2) * 1000
    ...
    # 恢复使用 blended_transform_factory 强制让下标绝对水平对齐
    sub_labels = ['(a)', '(b)', '(c)', '(d)']
    for i in range(4):
        # 决定使用哪个子图的 Y 坐标系作为基准
        if i in [0, 1]:
            y_ref_ax = ax[0]
            y_offset = -0.45  # 第一排距离 ax[0] 底部的下沉偏移量
        else:
            y_ref_ax = ax[2]
            y_offset = -0.25  # 第二排距离 ax[2] 底部的下沉偏移量

        # 生成混合变换：X 轴绑定到各自当前的子图 (居中)，Y 轴强制绑定到同一排第一个子图
        trans = blended_transform_factory(ax[i].transAxes, y_ref_ax.transAxes)

        ax[i].text(0.5, y_offset, sub_labels[i], transform=trans,
                   fontsize=14, ha='center', va='top', clip_on=False)
